...
export type ColumnTypesRaw = BigIntConstructor | BooleanConstructor | DateConstructor | NumberConstructor | StringConstructor;

export type ExtractNameBasedOffConstructor<T extends ColumnTypesRaw> = T extends BigIntConstructor ? "BigInt" : T extends BooleanConstructor ? "Boolean" : T extends DateConstructor ? "Timestamp" : T extends NumberConstructor ? "Number" : T extends StringConstructor ? "String" : never;

export type ColumnTypesStrings = "BigInt" | "bigint" | "Boolean" | "boolean" | "Date" | "date" | "Int" | "int" | "Number" | "number" | "String" | "string" | "Timestamp" | "timestamp"
...
export type ListAndFreezeType<T> = T extends string ? `frozen<${T}>` | `list<${T}>` | `list<frozen<${T}>>` | [`frozen<${T}>`] | [`list<${T}>`] | [`list<frozen<${T}>>`] | [T] : never
...
export type ConvertType<T extends FrozenlessTypes, Depth extends number = 10> =
    Depth extends never ? never :
    T extends ColumnTypesRaw ? `frozen<${Lowercase<ExtractNameBasedOffConstructor<T>>}>`
    : T extends ColumnTypesStrings ? `frozen<${Lowercase<T>}>`
    // @ts-expect-error -- Its fine
    : T extends ListableTypesNoFrozen ? `list<${ConvertType<ExtractType<T>, Decrement<Depth>>}>`
    : T extends [FrozenlessTypes] ? `list<${ConvertType<T[0], Decrement<Depth>>}>`
    : never;

export type ExtractType<T extends string> =
    T extends `list<${infer U}>` ? U
    : never;

export type ConvertToActualType<T> =
    T extends `list<${infer U}>` ? ConvertToActualType<U>[] :
    T extends `frozen<${infer U}>` ? ConvertToActualType<U> :
    T extends [infer U] ? ConvertToActualType<U>[] :
    T extends "BigInt" | "bigint" ? bigint :
    T extends "Boolean" | "boolean" ? boolean :
    T extends "Timestamp" | "timestamp" ? Date :
...
export type ExtractTypesFromCreateTable<T> = T extends Options<infer _Z, infer U, infer _P, infer _I>
    ? ConvertObjectToNormal<U> : T extends Table<infer T>
    ? ExtractTypesFromCreateTable<T> : never;
...
export type PublicGetReturnType<T, Fields extends (keyof T)[] | "*" = "*"> =
    Fields extends "*" ? NullifyStuff<T> :
...
export const convertToCorrectType = (type: string) => {
    if (mappings[type as keyof typeof mappings]) return mappings[type as keyof typeof mappings];
...
export const ExtractValueName = <T extends AllTypes>(type: T): AllTypes => {
    ...
    if (typeof type === "string") return convertToCorrectType(snakeifyString(type)).toString() as AllTypes;

    if (Array.isArray(type)) return `list<${ExtractValueName(type[0])}>`.toString() as AllTypes;
...
export const FreezeType = <T extends FrozenlessTypes>(type: T): ConvertType<T> => {
    if (typeof type === "string") {
        if (type.startsWith("frozen<") || type.startsWith("list<frozen<")) return type.toLowerCase() as ConvertType<T>;
