...
import type { AllTypes, ConvertObjectToNormal, ConvertToActualType, ExtractTypesFromCreateTable, Options, PublicGetReturnType } from "./createTableTypes.ts";
...
class Table<T> {
    ...
    public get options() {
        return this.#_options as T extends Options<infer Types, infer Columns, infer PrimaryKeys, infer IndexKeys> ? Options<Types, Columns, PrimaryKeys, IndexKeys> : never;
    }
    ...
    private checker() {
        ...
        if (reservedNames.includes(this.options.tableName)) {
            throw new Error(`The table name ${this.options.tableName} is a reserved name in Cassandra, you are required to change it to something else, here are some suggestions: ${this.options.tableName}_`);
        }
        ...
        if (this.options.primaryKeys.length === Object.keys(this.options.columns).length) {
            throw new Error("All columns are primary keys, this is not allowed, please have the minimal amount of primary keys");
        }

        // ? now for indexes, if there's more then 75% of the columns as indexes, we should probably throw a warning
        if (this.options.indexes && this.options.indexes.length > Math.floor(Object.keys(this.options.columns).length * 0.75)) {
            App.staticLogger.warn("You have more then 75% of the columns as indexes, this is not recommended, please have the minimal amount of indexes");
        }

        if (this.options.version !== undefined && (this.options.version === 0 || this.options.version === -1)) {
            throw new Error("[Internal Error] Sorry, the version you provided is an internally used version, this will conflict with migration scripts. Please use a non zero, non negative number for the version");
        }

        if (!tableAndColumnNameRegex.test(this.options.tableName)) {
            throw new Error(`The table name ${this.options.tableName} is invalid, it must match the regex ${tableAndColumnNameRegex}`);
        }
        ...
        for (const key of Object.keys(this.options.columns)) {
            ...
            if (reservedNames.includes(key)) {
                App.staticLogger.warn(`The column name ${key} is a reserved name in Cassandra, it is recommended to change it, we are prefixing it with an underscore`);
            }
        }
    ...
    public async migrateData<Data = Record<string, unknown>>(data: Data, version: number, where: { [key: string]: unknown }): Promise<Data> {
        if (this.options.migrationScripts) {
            const migration = this.options.migrationScripts[version];
            
            
            if (!migration) return data;
            App.staticLogger.debug(`[${this.options.tableName}] Migrating data from version ${version} to ${version + 1} due to ${migration.changes}`);

            const doWeHaveAllFields = Array.isArray(migration.fields) ? migration.fields.every((field) => data[this.snakeifyString(field) as never] !== undefined) : Object.keys(data as never).length === Object.keys(this.options.columns).length;
            ...
            if (doWeHaveAllFields) {
                ...
                if (Bun.deepEquals(migratedData, data)) {
                    ...
                    const opts = {
                        query: `UPDATE ${this.options.tableName} SET ${this.snakeifyString(this.versionName)} = ? WHERE ${Object.keys(where).map((key) => `${this.snakeifyString(key)} = ?`).join(" AND ")};`,
                    ...
                    App.staticLogger.debug(`[${this.options.tableName}] No changes were made to the data, but the version was updated to ${version + 1}`);
                } else {
                    const opts = {
                        query: `UPDATE ${this.options.tableName} SET ${migration.fields === "*" ? Object.keys(migratedData as never).map((f) => `${this.snakeifyString(f)} = ?`).join(", ") : migration.fields.map((f) => this.snakeifyString(f as string)).map((f) => `${f} = ?`).join(", ")}, ${this.snakeifyString(this.versionName)} = ? WHERE ${Object.keys(where).map((key) => `${this.snakeifyString(key)} = ?`).join(" AND ")};`,
                        params: [
                            ...Object.values(Object.fromEntries(Object.entries(migratedData as never).filter(([key]) => !Object.keys(where).includes(key)))),
                            version + 1,
                            ...Object.values(where)
                        ]
                    }
            ...
            const opts = {
                query: `SELECT ${migration.fields === "*" ? "*" : migration.fields.map((f) => this.snakeifyString(f as string)).join(", ")} FROM ${this.options.tableName} WHERE ${Object.keys(where).map((key) => `${this.snakeifyString(key)} = ?`).join(" AND ")};`,
            ...
            for (const [key, value] of Object.entries(first)) {
                ...
                const fixedType = Array.isArray(value) ? value.map((v) => {
                    if (Long.isLong(v)) {
            ...
            if (Bun.deepEquals(migratedData, finishedData)) {
                ...
                const optMigration = Object.fromEntries(Object.entries(migratedData as never).filter(([key]) => !Object.keys(where).includes(key)));
                
                const opts = {
                    query: `UPDATE ${this.options.tableName} SET ${migration.fields === "*" ? Object.keys(optMigration as never).map((f) => `${this.snakeifyString(f)} = ?`).join(", ") : migration.fields.map((f) => this.snakeifyString(f as string)).map((f) => `${f} = ?`).join(", ")}, ${this.snakeifyString(this.versionName)} = ? WHERE ${Object.keys(where).map((key) => `${this.snakeifyString(key)} = ?`).join(" AND ")};`,
                    params: [
                        ...Object.values(optMigration),
                        version + 1,
                        ...Object.values(where)
                    ]
                }
    ...
    >(filter: Partial<ConvertObjectToNormal<AdditionalColumns> | ExtractTypesFromCreateTable<this["options"]>>,
        options?: {
            ...
            fields?: Fields;
        }): Promise<PublicGetReturnType<(ConvertObjectToNormal<AdditionalColumns> & ExtractTypesFromCreateTable<this["options"]>), Fields> | null> {
        if (!options || !options.fields || options.fields === "*" || options.fields.length === 0 || options.fields.length === Object.keys(this.options.columns).length) {
            App.staticLogger.warn(`[${this.options.tableName}] You are fetching all fields, this is not recommended, please specify the fields you want to fetch`);
        }
        ...
        if (Array.isArray(options.fields)) {
            if (!options.fields.includes(this.versionName)) options.fields.push(this.versionName as string);
        ...
        const opts: {
            ...
            query: `SELECT ${(options.fields === "*" || !options.fields) ? "*" : options.fields.map((f) => this.snakeifyString(f as string))?.join(", ")} FROM ${this.options.tableName} WHERE ${Object.keys(filter).map((key) => `${this.snakeifyString(key)} = ?`).join(" AND ")}${options.allowFiltering ? " ALLOW FILTERING" : ""} LIMIT 1;`,
        ...
        let finishedData: Partial<PublicGetReturnType<(ConvertObjectToNormal<AdditionalColumns> & ExtractTypesFromCreateTable<this["options"]>), Fields>> = {};
        ...
        for (const [key, value] of Object.entries(first)) {
            ...
            // ? If the value is a array but the returned value is not an array (/null) we make it an array, this is due to cassandra returning null if the value is empty
            if (foundMappedType.value.toString().includes("list") && !value) {
                finishedData[this.convertBack(key) as keyof PublicGetReturnType<(ConvertObjectToNormal<AdditionalColumns> & ExtractTypesFromCreateTable<this["options"]>), Fields>] = [] as never;
            ...
            finishedData[this.convertBack(key) as keyof PublicGetReturnType<(ConvertObjectToNormal<AdditionalColumns> & ExtractTypesFromCreateTable<this["options"]>), Fields>] = fixedType;
        }
        
        if (this.versionName !== "") {
            const version = first[this.versionName];
            
            if (!version) finishedData = await this.migrateData(finishedData, 0, filter);
        ...
        return finishedData as PublicGetReturnType<(ConvertObjectToNormal<AdditionalColumns> & ExtractTypesFromCreateTable<this["options"]>), Fields>;
    }

    public async update<AdditionalColumns extends Record<string, AllTypes>, Filter extends Partial<(ConvertObjectToNormal<AdditionalColumns> | ExtractTypesFromCreateTable<this["options"]>)>>(filter: Filter, update: Omit<Partial<(ConvertObjectToNormal<AdditionalColumns> & ExtractTypesFromCreateTable<this["options"]>)>, keyof Filter>, options?: {
        /**
        ...
        const opts = {
            query: `UPDATE ${this.options.tableName} SET ${Object.entries(update).map(([key]) => `${this.snakeifyString(key)} = ?`).join(", ")}${options?.version ? `, ${this.versionName} = ${options.version}` : ""} WHERE ${Object.keys(filter).map((key) => `${this.snakeifyString(key)} = ?`).join(" AND ")};`,
    ...
    public async remove(filter: {
        // @ts-expect-error -- its fine
        [key in (T extends Options<infer _Columns, infer PrimaryKeys, infer _IndexKeys> ? PrimaryKeys : never)]: ConvertToActualType<this["options"]["columns"][key]>;
    }) {
    ...
    public async delete(filter: {
        ...
        const opts = {
            query: `DELETE FROM ${this.options.tableName} WHERE ${Object.keys(filter).map((key) => `${this.snakeifyString(key)} = ?`).join(" AND ")};`,
            params: Object.values(filter)
        }
    ...
    public async create<Data extends Partial<ExtractTypesFromCreateTable<this["options"]>>>(data: Data, options?: {
        /**
         * If you want to set a specific version for the tbale, this is somewhat dangerous but if for whatever reason you want to sure
         */
        version?: number;
        ...
        const opts = {
            query: `INSERT INTO ${this.options.tableName} (${Object.keys(data).map((key) => this.snakeifyString(key)).join(", ")}, ${this.snakeifyString(this.versionName)}) VALUES (${Object.keys(data).map(() => "?").join(", ")}, ${options?.version ? options.version : this.version});`,
    ...
    >(filter: Partial<ConvertObjectToNormal<AdditionalColumns> & ExtractTypesFromCreateTable<this["options"]>>,
        options?: {
            ...
            fields?: Fields;
            limit?: number;
        }): Promise<PublicGetReturnType<(ConvertObjectToNormal<AdditionalColumns> & ExtractTypesFromCreateTable<this["options"]>), Fields>[]> {
    ...
    private convertBack(str: string) {
        ...
        if (this.options.mode === "PascalCase") {
            return initalKey.split("_").map((part) => part[0]!.toUpperCase() + part.slice(1)).join("");
        }

        if (this.options.mode === "camelCase") {
            return initalKey.split("_").map((part, index) => index === 0 ? part : part[0]!.toUpperCase() + part.slice(1)).join("");
        }
    ...
    public toCQLCommand() {
        ...
        if (this.options.version !== undefined) {
            indexes.push(`CREATE INDEX IF NOT EXISTS ${this.options.tableName}_inx_${this.snakeifyString(this.versionName)} ON ${this.options.tableName} (${this.versionName});`);
        }
    ...
    public get columns() {
        return Object.entries(this.options.columns).map(([key, value]) => {
            ...
            if (reservedNames.includes(convertedString)) {
                console.warn(`The column name ${key} is a reserved name in Cassandra, it is recommended to change it, we are prefixing it with an underscore`);
    ...
    public get withOptions() {
        return Object.entries(this.options.with ?? {}).map(([key, value]) => this.handleWithOptions(this.snakeifyString(key), value));
    }

    public get indexes() {
        if (!this.options.indexes) return [];
    ...
    public get versionName() {
        return this.options.version === undefined ? "" : Array.isArray(this.options.version) ? this.options.version[0] : "int_tbl_ver";
    }
        ...
        switch (typeof value) {
            ...
            case "object": {
                if (!Array.isArray(value)) {
                    ...
                    builtValue = `{${Object.entries(value).map(([key, value]) => `'${this.snakeifyString(key)}': '${value}'`).join(", ")}}`;
