import { type ListAndFreezeType , ExtractValueName, reservedNames, type AllTypes, type Options } from "./createTableTypes.ts";
    ...
    switch (typeof value) {
        ...
        case "object": {
            if (!Array.isArray(value)) {
                ...
                builtValue = `{${Object.entries(value).map(([key, value]) => `'${key}': '${value}'`).join(", ")}}`;
...
>(options: Options<Types, Columns, PrimaryKeys, IndexKeys>) => {
    ...
    const built = [
        ...
        "",
        "",
        (options.indexes ?? []).length > 0 ? options.indexes?.map((index) => {
