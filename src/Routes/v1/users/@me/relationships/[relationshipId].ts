...
export default class RelationshipUser extends Route {
    ...
    public async postRelationships({
        ...
        if (!foundRelationship || foundRelationship.primaryUserId !== Encryption.encrypt(user.id) && foundRelationship.secondaryUserId !== Encryption.encrypt(user.id)) {
        ...
        if (relationshipFlagFields.has("Blocked") && !(bodyRelationshipFlag.has("Blocked") || bodyRelationshipFlag.has("None"))) {
            const notFound = errorGen.NotFound();
        ...
        if (bodyRelationshipFlag.count === 1) {
            ...
            switch (bodyRelationshipFlag.bits) {
                ...
                case relationshipFlags.Friend: {
                    ...
                    // ? If we've currently sent a fq, we cannot set them to friend
                    const ourFlags = new FlagUtils(whoAreWe ? foundRelationship.primaryUserFlags : foundRelationship.secondaryUserFlags, relationshipFlags);
                    ...
                    if (relationshipFlagFields.has("FriendRequest") && bodyRelationshipFlag.has("FriendRequest") || relationshipFlagFields.has("FriendRequest") && bodyRelationshipFlag.has("Friend")) {
                        const newFlags = {
                ...
                case relationshipFlags.Blocked: {
                    ...
                    const newFlags = {
                        primaryUserFlags: whoAreWe ? relationshipFlags.Blocked : relationshipFlagFields.has("Friend") ? relationshipFlags.None : relationshipFlagFields.bits,
                        secondaryUserFlags: whoAreWe ? relationshipFlagFields.has("Friend") ? relationshipFlags.None : relationshipFlagFields.bits : relationshipFlags.Blocked
                    };
                ...
                case relationshipFlags.None: {
                    ...
                    const newFlags = {
                        primaryUserFlags: whoAreWe ? relationshipFlags.None : relationshipFlagFields.has("Friend") ? relationshipFlags.None : relationshipFlagFields.bits,
                        secondaryUserFlags: whoAreWe ? relationshipFlagFields.has("Friend") ? relationshipFlags.None : relationshipFlagFields.bits : relationshipFlags.None
                    };
