...
class Client extends EventEmitter {
	...
	// @ts-expect-error
	public static tables: ListenerMap<string, Table<Options<unknown, unknown, unknown, unknown>>> = new ListenerMap();
	...
	public async connect(options: OurClientOptions) {
		...
		Client.tables.on("change", async (table) => {
			if (!table || !Client.instance.connected) {
				return;
			}
	...
	private async handleTable(keyspace: string, table: Table<_T>) {
		...
		if (
			!Bun.deepEquals(
				fixedPrimaryKeys,
				table.options.primaryKeys.map((k) =>
					Array.isArray(k) ? k.map((k) => table.snakeifyString(k)) : table.snakeifyString(k),
				),
			)
		) {
			console.log(
			...
			throw new Error(
				`The primary keys for the table ${table.snakeifyString(table.options.tableName)} have changed, we cannot change a tables primary key, please backup the data and recreate the table`,
			);
		}
		...
		if (
			...
			remoteIndexKeys.length > 1 &&
			remoteIndexKeys[0]!.name !==
				`${table.snakeifyString(table.options.tableName)}_inx_${table.snakeifyString(table.versionName)}`
		) {
			const answer = await rl.question(
				`The table ${table.snakeifyString(table.options.tableName)} has indexes that are not in the local table, would you like to remove them? [y/n] `,
			);
		...
		if (table.options.indexes) {
			for (const [name, target] of table.options.indexes.map((idx) =>
				Array.isArray(idx)
					? [idx[0], table.snakeifyString(idx[1])]
					: [
							`${table.snakeifyString(table.options.tableName)}_inx_${table.snakeifyString(idx)}`,
							table.snakeifyString(idx),
						],
			)) {
				if (!remoteIndexKeys.some((index) => index.name === name)) {
					const [, error] = await safePromise(
						this.connection.execute(
							`CREATE INDEX IF NOT EXISTS ${name} ON ${table.snakeifyString(table.options.tableName)} (${table.snakeifyString(target as string)})`,
						),
					);

					if (error) {
						throw new Error(
							`[${table.snakeifyString(table.options.tableName)}] Failed to create index ${name}: ${error.message}`,
						);
					}
			...
			for (const index of indexColumns) {
				...
				if (!indexKey) {
					const answer = await rl.question(
						`The index ${index.name} (target: ${index.target} | table: ${table.snakeifyString(table.options.tableName)}) is not in the local table, would you like to remove it? [y/n] `,
					);
		...
		if (table.versionName !== "") {
			...
			if (!versionColumn) {
				const answer = await rl.question(
					`[${table.options.tableName}] The version column ${table.versionName} is not in the remote table, would you like to add it? [y/n] `,
				);

				if (possibleYes.includes(answer)) {
					const [, error] = await safePromise(
						this.connection.execute(
							`ALTER TABLE ${table.snakeifyString(table.options.tableName)} ADD ${table.versionName} int`,
						),
					);
			...
			if (!versionIndex) {
				const answer = await rl.question(
					`[${table.options.tableName}] The version index ${table.snakeifyString(table.options.tableName)}_inx_${table.versionName} is not in the remote table, would you like to add it? [y/n] `,
				);

				if (possibleYes.includes(answer)) {
					const [, error] = await safePromise(
						this.connection.execute(
							`CREATE INDEX IF NOT EXISTS ${table.snakeifyString(table.options.tableName)}_inx_${table.snakeifyString(table.versionName)} ON ${table.snakeifyString(table.options.tableName)} (${table.versionName})`,
						),
					);

					if (error) {
						throw new Error(
							`Failed to create version index ${table.snakeifyString(table.options.tableName)}_inx_${table.snakeifyString(table.versionName)}: ${error.message}`,
						);
					}

					App.staticLogger.info(
						`Created version index ${table.snakeifyString(table.options.tableName)}_inx_${table.snakeifyString(table.versionName)}`,
					);
				}
			}
		}
		...
		for (const column of missingColumns) {
			...
			if (possibleYes.includes(answer)) {
				...
				if (error) {
					throw new Error(`Failed to add column ${found}: ${error.message}`);
				}
